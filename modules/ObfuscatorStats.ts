...
export default {
    async Get(): Promise<Obfuscator_Stats[]> {
        try {
            const result = await Database.GetTable<Obfuscator_Stats[]>("obfuscator_stats")
    ...
    async Update(stats: Obfuscator_Stats) {
        try {
            ...
            await Database.GetTable<Obfuscator_Stats>("obfuscator_stats", null, true).then(async res => {
    ...
    async Parse(limit = 7): Promise<number[]> {
        try {
            const result = await Database.GetTable<Obfuscator_Stats[]>("obfuscator_stats", null, false, limit + 1, true);
            ...
            let data: number[] = result.data.map(d => d.total_obfuscations).reverse();
